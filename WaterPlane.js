...
export class WaterPlane {
  ...
  constructor(
    ...
    // ─── Water mesh ────────────────────────────────────────────────────────────
    // Circular water mesh: use PlaneGeometry grid, then mask vertices to a circle
    const waterGeo = new THREE.PlaneGeometry(sizeX, sizeZ, resX - 1, resZ - 1)
    {
      const pos = waterGeo.attributes.position
      const radiusX = sizeX / 2
      const radiusZ = sizeZ / 2
      const radius = Math.min(radiusX, radiusZ)
    ...
    const raycastMesh = new THREE.Mesh(
      new THREE.CircleGeometry(Math.min(sizeX, sizeZ) / 2, 64),
